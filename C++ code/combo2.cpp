#include < dlib / opencv.h > #include < opencv2 / highgui / highgui.hpp > #include < opencv2 / opencv.hpp > #include < opencv2 / legacy / compat.hpp > #include < dlib / image_processing / frontal_face_detector.h > #include < dlib / image_processing / render_face_detections.h > #include < dlib / gui_widgets.h > #include < math.h > #include < stdlib.h > #include < string >
...
int main(int argc, char * * argv) {
    try {
...

        {
            ...
            cap >> frame[1];
            cap >> frame[2];
            std::vector < cv::Point > eye_leftPupilPoints;
            ...
            for (int i = 0; i < 4; i++) {
                ...
                if (shapes.size() == 0) {
                    ...
                    cv::Rect roi_left_eye_rect /*(rect_center_left_eye.x - 15, rect_center_left_eye.y - 18, 35, 30);*/
                    = cv::boundingRect(vec_pts_left_eye);
                    ...
                    cv::Rect roi_right_eye_rect /*(rect_center_right_eye.x - 15, rect_center_right_eye.y - 18, 35, 30);*/
                    = cv::boundingRect(vec_pts_right_eye);
